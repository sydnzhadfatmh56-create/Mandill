...
class TodoApp {
    ...
    addTask() {
        ...
        if (!text) {
            this.showNotification('لطفاً کاری بنویسید!', 'error');
            return;
        }

        if (text.length < 2) {
            this.showNotification('کار باید حداقل 2 حرف داشته باشد!', 'error');
            return;
        }
        ...
        this.render();
        this.showNotification('کار با موفقیت افزوده شد! ✅', 'success');
    }
    ...
    deleteTask(id) {
        this.showConfirmModal('آیا می‌خواهید این کار را حذف کنید؟', () => {
            ...
            this.saveTasks();
            this.render();
            this.showNotification('کار حذف شد! 🗑️', 'success');
        });
    }
